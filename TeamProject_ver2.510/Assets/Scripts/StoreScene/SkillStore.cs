using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Networking;
using SimpleJSON;

public class SkillStore : TowerStore
{
    protected override void Init()
    {
        buyReqURL = "http://pmaker.dothome.co.kr/pMaker_7Gi/SkillBuy.php";
        for (int i=0; i<(int)SkillType.SkillCount;i++)
        {
            if (!GlobalValue.SkillLvDic.ContainsKey((SkillType)i))
                GlobalValue.SkillLvDic.Add((SkillType)i, 0);

            GameObject obj = Instantiate(itemNodePrefab);
            ShopNode node = obj.GetComponent<ShopNode>();
            obj.transform.SetParent(itemSVContent.transform, false);
            node.InitData((SkillType)i,GlobalValue.SkillLvDic[(SkillType)i]);
        }

        StoreManager.Inst.RefreshMyInfo();
        RefreshItemList();
    }

    public override void BuyItem(int type)
    {
        if (isNetworkLock)
        {
            return;
        }

        if (type < 0 || nodeList.Length <= type)
            return;

        buyType = type;

        int price = 500 + (nodeList[type].lv * 50);

        string mess = "";
        ItemState state = ItemState.BeforeBuy;
        bool isBuyOK = false;

        if (nodeList != null && buyType < nodeList.Length)
        {
            state = nodeList[buyType].itemState;
        }
        switch (state)
        {
            case ItemState.BeforeBuy:
                if (GlobalValue.userGold < price)
                {
                    mess = "보유 골드가 모자랍니다";
                }
                else
                {
                    mess = "정말 구매 하시겠습니까";
                    isBuyOK = true;
                }
                break;
            case ItemState.Active:
                if (5 <= nodeList[type].lv)
                {
                    mess = "최고 레벨입니다";
                }
                else if (GlobalValue.userGold < price)
                {
                    mess = "레벨업에 필요한 보유 골드가 모자랍니다";
                }
                else
                {
                    mess = "정말 업그레이드 하시겠습니까";
                    isBuyOK = true;
                }
                break;
        }

        if (isBuyOK)
            ShowDlgBox(mess, TryBuyItem);
        else
            ShowDlgBox(mess);
    }

    public override void TryBuyItem()
    {
        bool isBuyOk = false;

        setLv.Clear();

        foreach (var node in nodeList)
        {

            setLv.Add(node.lv);

            if (node.type != buyType || 5 <= node.lv)
                continue;

            int cost = 500 + (node.lv * 50);
            if (GlobalValue.userGold < cost)
                continue;

            myGoldToServer = GlobalValue.userGold;
            myGoldToServer -= cost;
            setLv[node.type]++;
            isBuyOk = true;
        }

        if (isBuyOk)
        {
            if (setLv.Count <= 0)
                return;

            if (setLv.Count != nodeList.Length)
                return;

            JSONObject json = new JSONObject();
            JSONArray jArray = new JSONArray();
            for (int i = 0; i < setLv.Count; i++)
            {
                jArray.Add(setLv[i]);
            }
            json.Add("SkillLv", jArray);
            jsonStr = json.ToString();

            StartCoroutine(TowerStoreCo());
        }
    }

    protected override IEnumerator TowerStoreCo()
    {
        if (string.IsNullOrEmpty(jsonStr))
            yield break;

        if (string.IsNullOrEmpty(GlobalValue.myInfo.userID))
            yield break;

        WWWForm form = new WWWForm();
        form.AddField("Input_user", GlobalValue.myInfo.userID, System.Text.Encoding.UTF8);
        form.AddField("myGold", myGoldToServer);
        form.AddField("SkillLv", jsonStr, System.Text.Encoding.UTF8);
        isNetworkLock = true;

        UnityWebRequest request = UnityWebRequest.Post(buyReqURL, form);
        yield return request.SendWebRequest();

        if (request.error == null)
        {
            System.Text.Encoding enc = System.Text.Encoding.UTF8;
            string sz = enc.GetString(request.downloadHandler.data);

            if (sz.Contains("Success"))
                RefreshMyInfoCo();
        }
        else
        {
            Debug.Log(request.error);
        }

        isNetworkLock = false;
    }

    protected override void RefreshMyInfoCo()
    {
        if (!GlobalValue.SkillLvDic.ContainsKey((SkillType)buyType))
            return;

        GlobalValue.userGold = myGoldToServer;
        GlobalValue.SkillLvDic[(SkillType)buyType]++;

        nodeList[buyType].InitData((SkillType)buyType, GlobalValue.SkillLvDic[(SkillType)buyType]);

        RefreshItemList();
        StoreManager.Inst.RefreshMyInfo();
    }
}
